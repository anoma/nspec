    ...
    class JuvixPreprocessor:
        ...
        def __init__(self, config: MkDocsConfig):
            ...
            if not JUVIX_ENABLED:
                log.info(
                    "Juvix support is disabled. Set JUVIX_ENABLED to true to enable."
                )
                return
        ...
        def post_build(self) -> None:
            ...
            if not JUVIXCODE_CACHE_DIR.exists() or not list(
                JUVIXCODE_CACHE_DIR.glob("*")
            ):
                return

            sha_filecontent = (
                JUVIXCODE_HASH_FILE.read_text()
                if JUVIXCODE_HASH_FILE.exists()
                else None
            )
