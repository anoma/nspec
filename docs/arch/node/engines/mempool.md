...
These headers are ultimately what the [consensus](consensus_v1.md#consensus) decides on, in order to establish a total order of transactions.
...
In order to establish a total order of transactions, we use [Heterogeneous Paxos](consensus_v1.md#consensus) to decide on an ever-growing path through the DAG (for each Learner).
