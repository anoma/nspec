...
def prompt_boolean(prompt, allow_all=False):
    ...
    if allow_all:
        valid_responses.update({'yes all': 'all_yes', 'y all': 'all_yes',
                                'ya': 'all_yes', 'yesall': 'all_yes',
                                'na': 'all_no',
                                'no all': 'all_no', 'n all': 'all_no'}) # type: ignore
...
def to_snake(name):
    ...
    if "Package.juvix" in name:
        return name
    new_name = re.sub(r'(.)([A-Z][a-z]+)', r'\1_\2', name)  # Handle transition from lower/upper to Upper/lower case
    new_name = re.sub(r'([a-z0-9])([A-Z])', r'\1_\2', new_name)  # Handle transition from any lower/digit to upper case
...
def rename_dirs_recursive(root_dir):
    ...
    for path_object in root_directory.rglob('*'):
        ...
        cond = any(skip_pattern in old_path for skip_pattern in [".juvix-build", ".config", "tara", "docs/assets", "docs/.git", "docs/images"])
    ...
    def renaming(proceed_rename = True):
        ...
        proceed_directories = proceed_rename if proceed_rename in ['all_yes', 'all_no'] else \
            prompt_boolean("Proceed with renaming directories? (yes/no/yes all/no all): ", allow_all=True)
        
        if proceed_directories in [True, 'all_yes']:
            all_replacements.extend(replacements_dir.items())
        
        proceed_files = proceed_directories if proceed_directories in ['all_yes', 'all_no'] else \
            prompt_boolean("Proceed with renaming files? (yes/no/yes all/no all): ", allow_all=True)
        ...
        for old_path, new_path in all_replacements:
            ...
            rename_file = proceed_files if proceed_files in ['all_yes', 'all_no'] else \
                prompt_boolean(f"Rename {old_path} -> {new_path}? (yes/no/yes all/no all): ")
    ...
    proceed_rename = prompt_boolean("Proceed with renaming? (yes/no/yes all/no all): ", allow_all=True)
    ...
    def update_config_mkdocs(update_mkdocs = True):
        ...
        if update_mkdocs in [True, 'all_yes']:
            ...
            with open("mkdocs.yml", "w") as f:
                for _old_string, _new_string in [*replacements_dir.items(), *replacements_file.items()]:
    ...
    proceed_content_update = prompt_boolean("Proceed with updating mkdocs.yml? (yes/no/yes all/no all): ", allow_all=True)
    update_config_mkdocs(proceed_content_update)

    def delete_old_folders(proceed = True):
        if proceed in ['all_no']:
            return
        delete_old_dirs = proceed if proceed in ['all_yes', 'all_no'] else \
            prompt_boolean("Delete old directories? (yes/no/yes all/no all): ", allow_all=True)
    ...
    proceed_delete_old_dirs = prompt_boolean("Proceed with deleting old dirs? (yes/no/yes all/no all): ", allow_all=True)
    ...
    for path_object in root_directory.rglob('**/*.md'):
        ...
        cond = any(skip_pattern in path for skip_pattern in [".juvix-build", ".config", "tara", "docs/assets", "docs/.git", "docs/images"])
        if cond:
            continue

        process = process if process in ['all_yes', 'all_no'] else \
            prompt_boolean(f"Review: {path_object.as_posix()}\nProcess? (yes/no/yes all/no all):", allow_all=True)
        if process in [False]:
            continue
        ...
        # Replace content
        new_content = content
        for _old_string, _new_string in [*replacements_file.items(), *replacements_dir.items()]:
        ...
        cont = process if process in ['all_yes', 'all_no'] else \
            prompt_boolean("Continue? (yes/no/yes all/no all): ", allow_all=True)
        if cont in ['all_no']:
            break
